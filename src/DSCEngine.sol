 ...
 * @author notAltera
 *
 * The system is the design to be as minimal as possible,and have the token maintain a 1 token == $1 peg.
...
contract DSCEngine is ReentrancyGuard {
     ...
     * This function burns DSC and receive the underlying collateral in one transaction
     */
     ...
     * @notice follow CEI
     * @param amountDscToMint The amount of centralized stablecoin to mint
     * @notice they must havemore collateral value than the minimum threshold
     */
     ...
     * @notice You wil get a liquidation bonus for takint the users funds
    ...
    function getPriceFeedAddresses(address[] memory tokenAddressesArray, uint256 tokenAddressesIndex) external view returns(address){
        return s_priceFeeds[tokenAddressesArray[tokenAddressesIndex]];
    }

    function geTokenPriceFeed(address token) external view returns(address) {
        return s_priceFeeds[token];
    }

    function getCollateralTokensAddresses(uint256 tokenAddressesIndex) external view returns(address) {
        return s_collateralTokens[tokenAddressesIndex];
    }
    ...
    function simulateCollateralDeposited(address user, address token, uint256 collateralAmount) public {
        s_collateralDeposited[user][token] = collateralAmount;
    }

    function simulaterDscMinted(address user, uint256 amountDscMintedInUsd) public {
        s_DSCMinted[user] = amountDscMintedInUsd;
    }
}
